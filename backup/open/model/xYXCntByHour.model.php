 ...
 * @Last Modified time: 2019-05-07 18:37:39
 */
class XYXCntByHourModel {
    ...
    static function getCountDataByGameIdAndDay($gameid, $dayTime){
        $ym = date("Ym", $dayTime);
        $table = self::getTableByDay($ym);
        $startTime = $dayTime;
        $endTime = strtotime(date("Y-m-d H:i:s", $dayTime)." +1 day")-1;
        $sql = "select game_id,sum(total_time) as total_time,sum(active_user_num) as active_user_num,sum(new_reg_user) as new_reg_user from ".$table." where game_id=".$gameid." and a_time between ".$startTime." and ".$endTime." group by game_id ";
        return self::db()->getAllBySQL($sql);
    }
}