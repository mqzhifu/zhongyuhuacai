...
//        $content = file_get_contents("/data/www/zhongyuhuacai/spider/taohua_oumei.txt");
        $myPregRs = myPregMatchAll('/<tbody style="table\-layout:fixed;">(.*)<\/table>/isU',$content,1,0);
...
//            var_dump($matchTd[1][1]);
//            preg_match_all('/<b(.*)<\/b>/isU',$matchA[1][0],$matchB);

            preg_match_all('/href="(.*)"/isU',$matchA[1][0],$matchHref);


//            preg_match_all('/href="(.*)"/isU',$matchTd[1][4],$td);
//            var_dump($td);exit;
            preg_match_all('/div(.*)>(.*)</isU',$matchTd[1][2],$date);
//            var_dump($date[2][0]);exit;
