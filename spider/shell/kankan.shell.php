...
class KankanShell{
    function curlGetHtmlContent ($url,$host,$isSSl = 0,$cookid = "",$isFd = 0){
        return CurlLib::kankan($url,$host,$isSSl,$cookid,$isFd);
    }
...
//        $content = file_get_contents("/data/www/zhongyuhuacai/spider/taohua_oumei.txt");
        $myPregRs = myPregMatchAll('/threadlisttableid(.*)<\/table>/isU',$content,1,0);

        preg_match_all('/<tr(.*)<\/tr>/isU',$myPregRs,$match);
//        var_dump($match[1]);exit;
        $list = null;
        $i = 0;
//        $exceptTr = array("同城约炮 妹子陪睡 台湾uu裸聊直播 爱爱学院 免费18禁手游 末世王者 免费18禁手游");
        foreach ($match[1] as $k=>$tbody){
            ...
            preg_match_all('/<a(.*)<\/a>/isU',$matchTh[1][0],$matchA);

            preg_match_all('/href="(.*)"/isU',$matchA[0][1],$matchHref);
            preg_match_all('/>(.*)</isU',$matchA[0][1],$matchTitle);

//            var_dump($matchTitle);
//            var_dump($matchHref);exit;
