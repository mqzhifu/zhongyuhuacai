...
class WechatLib
{
	...
	const HTTP_API_BASE_URL_PREFIX = 'http://api.weixin.qq.com'; //http 模式API接口URL需要使用此前缀
	...
	const CARD_CODE_DEPOSIT               = '/card/code/deposit?';
	const CARD_CODE_CHECKCODE             = '/card/code/checkcode?';
    ...
    const API_WEATHER_URL  = 'http://api.map.baidu.com/telematics/v3/weather?'; //百度天气API
    const API_RATE_URL = "http://api.uihoo.com/currency/currency.http.php?";//汇率转换API
    const API_TRANSLATION_URL = "http://openapi.baidu.com/public/2.0/bmt/translate?";//在线翻译api
   // const API_MAP_URL="http://api.map.baidu.com/place/search?";//百度地图周边搜索api
	//数据分析接口
	...
	public $wx_login_uname;
	public $wx_id;


	public $receive_db_id = 0;


	static function getMsgType(){
		return array(
			self::MSGTYPE_TEXT,self::MSGTYPE_IMAGE,self::MSGTYPE_LOCATION,self::MSGTYPE_LINK,self::MSGTYPE_MUSIC,self::MSGTYPE_NEWS,self::MSGTYPE_VOICE,self::MSGTYPE_VIDEO,self::MSGTYPE_EVENT
		);
	}
	...
	public function __construct($options)
	{
		...
		$this->wx_login_uname = isset($options['wx_login_uname'])?$options['wx_login_uname']:false;
		$this->wx_id = isset($options['wx_id'])?$options['wx_id']:false;
	...
	*/
	 ...
	 *
	 * 回复微信服务器, 此函数支持链式操作
	 * Example: $this->getRev('msg tips')->reply();
	...
	public function reply($msg=array(),$return = false)
	{
		...
		$data = array(
			...
			'receive_log_id'=>$this->receive_db_id,
		);
		 wxReplyLogModel::db()->add($data);
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_post($url,$param,$post_file=false){
		...
		if (is_string($param) && !$post_file){
			$strPOST = $param;
		}else if(is_string($param) && $post_file) {
			$strPOST['media'] = $param;
...
//		var_dump($result);exit;
     ...
     * api天气预报
     */
    public function queryWeather($city){

        $ak = "ECe3698802b9bf4457f0e01b544eb6aa";//自己百度ak
        $data['location'] = $city;
        $data['output'] = "json";
        $data['ak'] = $ak;
        $result = $this->http_post(self::API_WEATHER_URL,$data);
    ...
    public function queryRate($from,$to){
        ...
        $data['to'] = $to;
        $data['form'] = "json";
        $result = $this->http_post(self::API_RATE_URL,$data);
        if($result){
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     *  $text 翻译的文本
     */
    public  function queryTranslation($to,$text){
        $akey = "0ZinyoGLHU9zZkfuBpfzKhuB";//开发者在百度开发者中心注册得到的授权API key
        $data['client_id'] = $akey;
        $data['from'] = "auto";
        $data['to'] = $to;
        $data['q'] = $text;
        $result = $this->http_post(self::API_TRANSLATION_URL,$data);
        if($result){
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     *   errmsg 	错误信息
     *   succ_code 	成功个数
     *   duplicate_code 	重复个数。重复导入的code会自动被过滤。
     *   fail_code 	失败个数
     */
    /*public function customChannelCode($card_id='',$number=0) {
        $code = array();
        for($k=0;$k<=$number;$k++){
            $code_str='';
            for($i=0;$i<3;$i++){
                $code_str.=mt_rand(1000,9999);
            }
            $code[] = $code_str;
        }
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::HTTP_API_BASE_URL_PREFIX . self::CARD_CODE_DEPOSIT . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     *
     */
    /*public function inspectCode($card_id='',$code) {

        $data = array('code' => $code);
        if ($card_id)
            $data['card_id'] = $card_id;
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::HTTP_API_BASE_URL_PREFIX . self::CARD_CODE_CHECKCODE . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
...
class ErrorCode
{
	...
	function makeKFAcoount($private_wx_account){
		if(!$private_wx_account)
			return false;

		$wx_login_uname_pre = explode("@",$private_wx_account);

		$kf_account = $wx_login_uname_pre[0] . "@" . $this->wx_id;
		return $kf_account;
	}
}
