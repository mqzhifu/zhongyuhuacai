...
class SwooleWebSocketMatchLib{
    ...
    private $_config = array(
        ...
        'reactor_num'=>4,//最好和CPU数对应,调节poll线程的数量
...
//        'ssl_key_file'=>'/soft/nginx/conf/heyshell.key',
    ...
    public function run(){
        ...
        LogLib::wsWriteFileHash(["swoole version:",swoole_version()]);
...
//        $server = new swoole_websocket_server($this->_ip, $this->_port ,SWOOLE_PROCESS,SWOOLE_TCP|SWOOLE_SSL);
        ...
        //监听manager进程启动
        $server->on('managerStart','onManagerStart');
        ...
        $table = new swoole_table($this->_config['max_conn']);
        $table->column('uid', swoole_table::TYPE_STRING, 20);
        $table->create();
...
//        $server->addListener($GLOBALS['server']['http_bind_ip'],$GLOBALS['server']['http_port'],SWOOLE_SOCK_TCP);
...
//
        ...
        $server->on('pipeMessage', function($server, $src_worker_id, $data) {
            LogLib::wsWriteFileHash(" pipeMessage :{$server->worker_id} message from #$src_worker_id: $data\n");
            swoole_timer_clear($data);
        });


        LogLib::wsWriteFileHash(['new swoole_websocket_server:',$server]);

        $processName =$GLOBALS['server'][SERVER_CONF_KEY]['process_name']."_".ENV;
        LogLib::wsWriteFileHash(" start:swoole demon process.... $processName");
...
function getAppInfo(){
    $apps = AppModel::db()->getAll(" status = 1 ");
    if(!$apps){
        exit(" all app is off");
    }
    ...
    $app_types = [];
    foreach ($apps as $k=>$v) {
        $types = AppTypeModel::db()->getAll(" status = 1 and app_id = ".$v['id']);
        if($types){
            $app_types[$v['id']] = $types;
        }
    }

    if(!$app_types){
        exit(" all app_type is off");
    }
...
function onManagerStart(){
    $app_types = getAppInfo();

    $gameMatch = new GameMatchService();
    foreach ($app_types as $k=>$v) {
        foreach ($v as $k2=>$v2) {
//            $para = [$k,$v2['id']];
            //这里还没有做加锁操作，待处理
            $timerId = swoole_timer_tick(1000,[$gameMatch,'matchRealUser'],$k,$v2['id'] );
            LogLib::wsWriteFileHash([" add app type timer:",$timerId,'appid:'.$k,"typeid:".$v2['id']]);
        }
    }

//    LogLib::wsWriteFileHash([" add app type timer:",$timerId,'appid:'."1","typeid:"."1"]);
}

function myOnTask($serv,  $task_id,  $src_worker_id,  $data){
    LogLib::wsWriteFileHash(["in myOnTask", $task_id,  $src_worker_id,  $data]);
...
function onClose(){
    ...
    $class = new LoginCtrl($fd,'login','onClose');
    $out_info = $class->onClose();
...
function onShutdown($server){
    ...
    foreach ($app_types as $k=>$v) {
        foreach ($v as $k2=>$v2) {
            //这里还没有做加锁操作，待处理
            $lockKey = RedisPHPLib::getAppKeyById($GLOBALS['rediskey']['matchedUserLock']['key'],$k."_".$v2['id']);
            $rs = RedisPHPLib::delLock($lockKey);
            LogLib::wsWriteFileHash([" del lockey :$lockKey",$rs]);
        }
    }
...
function onMessage(){
    ...
    LogLib::wsWriteFileHash(['code',$code]);
    //根据消息头，消息号，映射对应的执行方法
    $config = [];
