...
消息retry机制
-
正常消息者的callback函数是，处理消息的。
但有些特殊的情况，比如：拿到的消息发送有问题，这时候要怎么办？
1告诉类库，这条消息我收到了，但是消息内容不对，丢掉  
2没准该消息还依赖其它节点计算，要不，我们再等一等。

先说第2种情况，这就是消息的retry机制，也就是：暂时不处理，过一会再处理。
```java
...
在callback函数中，只需要抛出 RetryException  异常即可    
至于，想要重试的时间次数，自定义一个数组即可  
如：array(2,6) ，代表重试2次，第一次是2秒之后，第二次是6秒之后  

如果在2次之后，该消息依然没有被消费，那么该消息将会被类库丢掉  
...
如果发现某条消息内容就是错的，不需要再重试了，已经确定可以丢掉了  
只需要抛出 RejectMsgException 即可  

执行超时
-
当消费者，在 callback函数中，处理消息的时候，可能会遇到某些连接阻塞  
如：从MYSQL读取一些数据特别大的时候、连接redis没响应了  
遇到这样的情况，PHP单进程单线程模式，就会导致队列里的消息积压（没有其它消息者）  
类库会有超时KILL机制，一但超过60没有响应后，当前进程会被KILL掉  
KILL掉后，正在处理的那消息依然会返回到rabbitmq 队列中，不会丢失  


consumer启动方式
-
建议配合supervisor使用  
因为，PHP是单进程单线程模式，如果一但超时，类库会直接给KILL掉  
如果此时，不重启的话，消息会积压  

服务器环境说明
=
172.19.162.186：线上，负载机入口，后面还有3台节点机  
172.19.113.249：测试，负载机入口，后面还有3台节点机  
