...
class ProductService
{
    ...
    const ORDER_TYPE = array(
        1 => array('id' => 1, "name" => '默认', 'field' => "id"),
        2 => array('id' => 2, "name" => '新品', 'field' => "id"),
        3 => array('id' => 3, "name" => '销量', 'field' => "user_buy_total"),
        4 => array('id' => 4, "name" => '价格', 'field' => 'lowest_price'),
    );
...
//        $total = count($list);
    ...
    function searchUidsByKeywordUseDbWhere($keyword){
        ...
        $productWhere = " title like '%$keyword%' or subtitle like '%$keyword%' or `desc` like '%$keyword%' ";
        $productList = ProductModel::db()->getAll($productWhere,null, " id ");
        if(!$productList){
            $where .= " and 0";
        }else{
            $pids = "";
