...
class PlayedGamesModel {
	...
	static function getPartDataByGameIdAndDayTime($gameid, $dayTime){
		$startTime = $dayTime;
		$endTime = strtotime(date("Y-m-d H:i:s", $dayTime)." +1 day")-1;
		$sql = "select uid,a_time,e_time,src from ".self::$_table." where game_id=".$gameid." and a_time between ".$startTime." and ".$endTime;
		return self::db()->getAllBySQL($sql);
	}

	static function getPartDataByGameIdAndTimeInterval($gameid, $startTime, $endTime){
		$sql = "select uid,a_time,e_time,src from ".self::$_table." where game_id=".$gameid." and a_time between ".$startTime." and ".$endTime;
		return self::db()->getAllBySQL($sql);
	}
	static function getSrcDesc(){
    	return array(
    		self::$_src_unknown=>"未知",
