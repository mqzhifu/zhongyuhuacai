...
class SocialImMsgCtrl extends BaseCtrl{
    function index(){
...
//        $this->assign("screenDesc",GamesModel::getScreenDesc());
        ...
        $this->display("social/im_msg_list.html");
    }
    ...
    function getData(){
        ...
        $cnt = ImMsgModel::db()->getCount($where);
...
//            var_dump($where);exit;
            $data = ImMsgModel::db()->getAll($where . $order);
            ...
            foreach($data as $k=>$v){
                $row = array(
                    ...
                    $v['from_uid'],
                    $v['to_uid'],
                    ImMsgModel::getTypeDescByKey($v['type']),
                    get_default_date($v['to_uid_del_time']),
                    $v['session_id'],
                    $v['content'],
