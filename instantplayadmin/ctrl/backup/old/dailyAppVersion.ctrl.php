...
class DailyAppVersionCtrl extends BaseCtrl{
    function index(){
        if(_g("getlist")){
            $this->getList();
        }
        $this->display("daily/app_version_list.html");
    }
    ...
    function getData(){
        ...
        $cnt = AppVersionModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = AppVersionModel::db()->getAll($where . $order);
