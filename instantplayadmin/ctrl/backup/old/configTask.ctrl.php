...
class configTaskCtrl extends BaseCtrl{
    function index(){
        if(_g("getlist")){
            $this->getList();
        }
        $this->display("config/task_list.html");
    }
    ...
    function getData(){
        ...
        $cnt = TaskConfigModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = TaskConfigModel::db()->getAll($where . $order);
