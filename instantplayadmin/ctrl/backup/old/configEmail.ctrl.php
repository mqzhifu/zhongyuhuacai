...
class configEmailCtrl extends BaseCtrl{
    function index(){
        if(_g("getlist")){
            $this->getList();
        }
        $this->display("config/email_list.html");
    }
    ...
    function getData(){
        ...
        $cnt = EmailRuleModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = EmailRuleModel::db()->getAll($where . $order);
