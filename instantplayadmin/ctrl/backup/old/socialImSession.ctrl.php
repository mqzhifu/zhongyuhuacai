...
class SocialImSessionCtrl extends BaseCtrl{
    function index(){
...
//        $this->assign("screenDesc",GamesModel::getScreenDesc());
        ...
        $this->display("social/im_session_list.html");
    }
    ...
    function getWhere(){
        ...
        if($u_from = _g("u_from")){
            $u_from .= ":00";
            $where .= " and u_time >= '".strtotime($u_from)."'";
        }

        if($u_to = _g("u_to")){
            $u_to .= ":59";
            $where .= " and u_time <= '".strtotime($u_to)."'";
        }
    ...
    function getData(){
        ...
        $cnt = ImSessionModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $sort = array(
                ...
                'to_uid',
                '','',
                'from_uid_unread','to_uid_unread','u_time',
                'a_time',
            );
...
//            var_dump($where);exit;
            $data = ImSessionModel::db()->getAll($where . $order);
