...
class DailyReportUserCtrl extends BaseCtrl{
    function index(){
        if(_g("getlist")){
            $this->getList();
        }

        $this->display("daily/report_user_list.html");
    }
    ...
    function getData(){
        ...
        $cnt = ReportUserModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = ReportUserModel::db()->getAll($where . $order);
