...
class LinkGamePowerCtrl extends BaseCtrl
{
	...
	function index()
	{
		...
		$this->display("app_manager/linkGamePower/index.html");
	}
	...
	{
		$this->assign('roleSearch', ['全部','链接游戏','微信游戏']);
		$this->display("app_manager/linkGamePower/account.html");
	}
        ...
        if ($iTotalRecords){
            ...
            foreach ($roles as $r) {
    			$row = array(
                    ...
                    '<a href="/app_manager/no/linkGamePower/modify/id='.$r['id'].'" class="btn btn-xs default blue" data-id="'.$r['id'].'"><i class="fa fa-file-text"></i> 修改</a>',
                );
                $records["data"][] = $row;
        	}
