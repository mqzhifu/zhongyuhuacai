 ...
 * @Last Modified time: 2019-05-31 17:32:07
 */
...
class MoneyDivideCtrl extends BaseCtrl
{
	...
	function index()
	{
		...
		$this->display("/finance/purchase/money_divide/index.html");
	}
	...
	function divideSetting()
	{
        ...
        $finance = FinanceModel::db()->getRow("game_id = $game_id and finance_type=".FinanceModel::$type_purchase);
        ...
        $this->assign("accountType", OpenUserModel::getAccountDescs());
        $this->display("/finance/purchase/money_divide/divide_setting.html");
	}
	...
	function save()
	{
		...
		$data = [
            'finance_type'=>FinanceModel::$type_purchase,
			'game_id'=>_g("game_id"),
		...
		if (!$finance) {
			...
			if (FinanceModel::db()->add($data)) {
                ...
                $result = $settleAccountService->recomputeDivideAndTax($game_id, FinanceModel::$type_purchase);
                if($result){
			...
			if (FinanceModel::db()->update($data, "game_id = $game_id and finance_type = ".FinanceModel::$type_purchase." limit 1")) {
	...
	function getList()
	{
        ...
        $sql = "select count(*) as cnt from (select gu.id as game_id,gu.game_name,gu.type, gu.company, gu.contact,f.divide,f.contract_start_date,f.contract_end_date,f.u_time from game_finance as f right join( select g.id, g.name as game_name, o.type, o.company,o.contact from games as g left join open_user as o on g.uid=o.uid) as gu on f.game_id = gu.id ) as c where $where";
        ...
        if ($iTotalRecords){
            ...
            foreach($data as $k=>$v){
                ...
                $row = array(
                    ...
                    '<a href="/finance/purchase/moneyDivide/divideSetting/game_id='.$v['game_id'].'" class="btn btn-xs default blue" data-id="'.$v['game_id'].'"><i class="fa fa-file-text"></i> 编辑</a>',
                );
	...
	function getWhere()
	{
        ...
        // $where .= " and finance_type=".FinanceModel::$type_purchase." ";
        return $where;
	}
}