 ...
 */
class gameInquireCtrl extends BaseCtrl{
    public function index(){
        if (_g("getlist")) {
            $this->getList();
        }
        $this->display("/app_share/gameInquire/index.html");
    }
    ...
    public function getList(){
        ...
        if (0 != $iTotalRecords){
            ...
            foreach($data as $k=>$v){
                ...
                $gameId = $v['game_id'];
                $selectSql = "SELECT COUNT(DISTINCT(uid)) AS uid_cnt FROM share WHERE game_id = {$gameId} AND type = 80 AND a_time >= {$beginTime} AND a_time <= {$endTime} ;";
                ...
                $num = ShareModel::db()->getCount(" game_id = $gameId AND type = 80 AND a_time >= {$beginTime} AND a_time <= {$endTime} ");
                $records["data"][] = array(
                    $v['date_new'],
                    $v['game_id'],
                    ...
                    '<span style="font-weight: bold">'.$counts[0]['click_count'].'</span>',
                    '<span style="font-weight: bold">'.$counts[0]['login_count'].'</span>',
                    '<span style="font-weight: bold">'.$counts[0]['download_count'].'</span>',
                    '---'
                );
            }
        }
