 ...
 * @Last Modified time: 2019-01-31 15:19:01
 */
...
class LogChestCtrl extends BaseCtrl
{
    function index() {
        if(_g("getlist")){
            $this->getList();
		...
		$this->assign("typeDesc",ChestLogModel::getTypeDesc());        

        $this->display("log/chest_list.html");
    ...
    function getList() {
        ...
        $cnt = ChestLogModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = ChestLogModel::db()->getAll($where . $order);
            ...
            foreach($data as $k=>$v){
                $row = array(
                    ...
                    ChestLogModel::getTypeDescByKey($v['reward_type']),
                    $v['reward_goldcoin'],
                    "",
                );
