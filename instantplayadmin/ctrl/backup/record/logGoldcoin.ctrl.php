...
class LogGoldcoinCtrl extends BaseCtrl{
    function index(){
        if(_g("getlist")){
            $this->getList();
        }

        $this->display("log/goldcoin_list.html");
    ...
    function getData(){
        ...
        $cnt = GoldcoinLogModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = GoldcoinLogModel::db()->getAll($where . $order);
