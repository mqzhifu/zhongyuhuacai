...
class LogAPICtrl extends BaseCtrl{
    function index(){
        if(_g("getlist")){
            $this->getList();
        }

        $this->display("log/api_list.html");
    }
    ...
    function getData(){
        ...
        $cnt = AccesslogModel::db()->getCount($where);
        ...
        if ($iTotalRecords){
            ...
            $data = AccesslogModel::db()->getAll(" $where limit $iDisplayStart,$end");
