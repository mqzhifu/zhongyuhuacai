 ...
 * @Last Modified time: 2019-06-17 15:02:44
 */
...
class ControlCtrl extends BaseCtrl
{
	...
	function index()
	{
		$this->assign("adTypeDesc", OpenAdvertiseModel::getAdvertiseTypeDesc());
        ...
        if ($iTotalRecords){
            ...
            $data = AdCtrlModel::db()->getAllBySql("
				...
				FROM
					...
					LEFT JOIN games ON ac.game_id = games.id
            ...
            foreach ($data as $val) {
            	...
            	if (PCK_AREA == 'en') {
            		$row = array(
	                    $status,
	                    OpenAdvertiseModel::getAdvertiseTypeDesc()[$val['ad_type']],
	                    $val['name'],
	...
	function getWhere()
	{
		...
		if ($game_name = _g("game_name")) {
			$games = GamesModel::db()->getAllBySql("select id from games where name like '%$game_name%'");
			$ids = array_column($games, 'id');
            $idstr = implode(',', $ids);
            if (!$idstr) {
            	$idstr = 0;
            }
			$where .= " and game_id in ($idstr)";
		}
	...
	function getConfig()
	{
		...
		$games = OpenAdvertiseModel::db()->getAllBySql("select g.id, g.name from (select * from open_advertise where $where ) as a inner join (select id, name from games where is_online = 1) as g on a.game_id = g.id group by g.id");

		$where = "game_id = {$record['game_id']} and advertise_type = {$record['ad_type']}";

		$adIds = OpenAdvertiseModel::db()->getAllBySql("select id from open_advertise where $where");

		$this->outputJson(200, "succ", ['record'=>$record, 'games'=>$games, 'adIds'=>$adIds]);	
	}
	...
	function getAdIds()
	{
		...
		if (!$gameId = _g('gameId')) {
			$this->outputJson(0, "请选择游戏");
		}

		$where = "game_id = $gameId and advertise_type = $adType";
	...
	function save()
	{
		...
		$now = time();
		$where = " type=1 and ad_type=$adType and ctrl_type=$ctrlType and ( time_ctrl_type = 1 or (time_ctrl_type = 2 and `from` < $now and `to` > $now)) ";
