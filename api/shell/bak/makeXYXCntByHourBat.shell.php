...
class MakeXYXCntByHourBat{
    ...
    public function run($attr){
        ...
        echo "MakeXYXCntByHourBat-----------start-----\n";
        echo "脚本执行时间：".date("Y-m-d H:i:s", $addTime)."\n";
        for($dayTime = $startDayTime; $dayTime <= $nowDayTime; $dayTime += 3600){
            $this->importData2($dayTime);
        }
        echo "MakeXYXCntByHourBat-----------end-----succ\n";
    ...
    private function importData2($time){
        ...
        $dataTime = strtotime(date("Y-m-d H:00", $time-3600));
